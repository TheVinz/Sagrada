...
public class CliModelTest {
    ...
    @Test
    public void shouldLoadPrivateObjectiveCard(){
        outContent = new ByteArrayOutputStream();
        cliModel.loadPrivateObjectiveCard(color);
    }
    @Test
    ...
    public void shouldLoadToolCards(){
        outContent=new ByteArrayOutputStream();   //stampo il numero o il nome della ToolCard?
        cliModel.loadToolCards(toolCard);
    }
    @Test
    public void shouldRefilledDraftPool(){
        outContent = new ByteArrayOutputStream();
        cliDisplayer.printDraftPool();
        cliModel.refilledDraftPool(draftPoolv,draftPoolc);
    }
    @Test
    public void shouldLoadPublicObjectiveCards(){
        outContent = new ByteArrayOutputStream();
        cliModel.loadPublicObjectiveCards(publicObjectiveCardIds);
    }
    @Test
    public void shouldLoadWindowFrameChoice(){
        outContent = new ByteArrayOutputStream();
        cliModel.loadWindowFrameChoice(reps,favor);
    }
    @Test
    public void shouldToolCardUsed(){
        cliModel.toolCardUsed(1,1,1);
        Integer check = 4;
        assertEquals(check,cliPlayerState[0].getFavorTokens());
    }
    @Test
    public void shouldLoadPlayers(){
        cliModel.loadPlayers(names,ids,reps,favor);
        assertEquals(1,cliState.getCliPlayerState("gabriele").getId());
    }

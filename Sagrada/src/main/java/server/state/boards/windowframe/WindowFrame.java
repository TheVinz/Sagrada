		...
		for(int i=0; i<ROWS; i++){
			for(int j=0; j<COLUMNS; j++){
				switch(windowFrameList.getRep().charAt(i*COLUMNS+j)){
					case 'y':
						cells[i][j]=new WindowFrameCell(YELLOW,i,j);
						break;
					case 'b' :
						cells[i][j]=new WindowFrameCell(BLUE,i,j);
						break;
					case 'g' :
						cells[i][j]=new WindowFrameCell(GREEN,i,j);
						break;
					case 'r' :
						cells[i][j]=new WindowFrameCell(RED,i,j);
						break;
					case 'p' :
						cells[i][j]=new WindowFrameCell(PURPLE,i,j);
						break;
					case '1' :
